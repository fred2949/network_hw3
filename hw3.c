...
void pcap_callback(u_char *dumpfile, const struct pcap_pkthdr *header, const u_char *packet)
{
    ...
    // time info
    printf("封包擷取時間: %s", ctime((time_t *) &header->ts.tv_sec));
...
void parse_ethernet(const u_char *packet)
{
    ...
    printf("┌────────────────────────────────────┐\n");
    printf("│ 目標 MAC 位址:    %17s│\n", mac_dst_addr);
    printf("│ 來源 MAC 位址:    %17s│\n", mac_src_addr);
    printf("└────────────────────────────────────┘\n");
    if (eth_type < 1500)
        printf(" Length:            %5u\n", eth_type);
    else
